...
{	
	//first try to find high res texture
	std::map< std::string, CTextureFromBMP* >::iterator itTexture
		//= this->m_map_TexNameToTexture.find("High_" + textureFileName);
		= this->m_map_TexNameToTexture.find("High_" + textureFileName);
	if (itTexture == this->m_map_TexNameToTexture.end() || !(itTexture->second->textureLoadStatus == CTextureFromBMP::IS_READY_TO_BE_COPIED_TO_GPU
		|| itTexture->second->textureLoadStatus == CTextureFromBMP::IS_IN_GPU))
	{
		itTexture = this->m_map_TexNameToTexture.find(textureFileName);
		if (itTexture == this->m_map_TexNameToTexture.end())
		{
			return 0;
		}
